...
func (s *Storage) SaveUser(ctx context.Context, newUser models.User) (userId int64, err error) {
    ...
    `
	var id int64
	err = s.db.QueryRowContext(ctx, query, newUser.Email, newUser.PasswordHash, newUser.Name, newUser.DateOfBirth.Format("2006-01-02")).Scan(&id)
	if err != nil {
...
func (s *Storage) User(ctx context.Context, email string) (*models.User, error) {
	smtp, err := s.db.Prepare("SELECT id, email, password_hash, name, date_of_birth, create_date " +
	...
	err = row.Scan(&user.Id, &user.Email, &user.PasswordHash, &user.Name, &user.DateOfBirth, &user.CreateDate)
	if err != nil {
...
func (s *Storage) UserById(ctx context.Context, id int64) (*models.User, error) {
	smtp, err := s.db.Prepare("SELECT id, email, password_hash, name, date_of_birth, create_date FROM users WHERE id = $1")
	if err != nil {
		return nil, fmt.Errorf("internal: %w", storage.ErrorSqlSyntax)
	}
